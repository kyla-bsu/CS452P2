...
void test_cmd_free(void) {
     ...
     cmd_free(cmd);
     // No assertion needed, just ensure no memory leaks or crashes
 }

 
 void test_init_history(void) {
     init_history();
     // No assertion needed, just ensure no crashes
 }

 void test_add_history_entry(void) {
     init_history();
     add_history_entry("ls -l");
     // No assertion needed, just ensure no crashes
 }
...
int main(void) {
  ...
  RUN_TEST(test_init_history);
  RUN_TEST(test_add_history_entry);
  RUN_TEST(test_print_history);
  //RUN_TEST(test_exit_shell);
  return UNITY_END();
}
